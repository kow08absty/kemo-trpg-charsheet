Vue.component('custom-footer', {
	...
	<footer>
		...
		<div id="using" v-show="!pdf_capturing">
			<h2>入力にあたっての注意</h2>
			<ol>
				<li v-for="text in using" v-html="text"></li>
			</ol>
		</div>
	...
	data: function() {
		return {
			...
			using: [
				"「大きさ」を変更すると「げんき」と「ジャパリコイン」の枚数を自動で上書き修正するよ。<br />アイテムの購入に「ジャパリコイン」を使っていたら、その枚数を改めて計算してね。",
				"「スキルタイプ」を変更すると「キラキラの最大値」と「野生解放の上限値」を自動で上書き修正するよ。<br />「野生解放の上限値」が変化するアイテム（ネックレスなど）を持っていたら、改めて調整してね。",
				"「アイテム」プリセットを実装したよ。初期アイテムの中から、購入したいものを選ぼう。<br />「ジャパリコイン」を手動で書き換えていなければ、必要なコインが引き算されるよ。初期アイテムの名前を直接入力しても、アイテムの効果が自動で入力されるよ。",
